...
#let resume(
  ...
  show heading.where(level: 1): it => [
    #set text(size: 1.25em, weight: "regular")
    #set align(left)
    #set block(above: 1em)
  ...
  show heading.where(level: 2): it => {
    set text(color-darkgray, size: 1.05em, style: "normal", weight: "bold")
    it.body
  }

  show heading.where(level: 3): it => {
    set text(rgb("#666"), size: 0.95em, weight: "regular")
  ...
  let positions = {
    set text(accent-color, size: 1em, weight: "regular")
    align(center)[
      #__apply_smallcaps(
...
#let resume-item(body) = {
  set text(size: 1em, style: "normal", weight: "light", fill: color-darknight)
